...
export const studentAiBridge = {
  ...
  generateQuiz(body: { studentId: string; subjectId: string; chapters: string[]; noteIds: string[]; noteUrls?: Record<string, string>; questionCount: number; seed: string }) {
    return requestInternal<{ subject_id: string; chapters: string[]; questions: Array<{ text: string; options: string[]; correct_index: number; explanation: string; chapter: string }> }>("quizzes/generate", {
      method: "POST",
      body: JSON.stringify({ student_id: body.studentId, subject_id: body.subjectId, chapters: body.chapters, note_ids: body.noteIds, note_urls: body.noteUrls ?? {}, question_count: body.questionCount, seed: body.seed }),
    });
  },
};
